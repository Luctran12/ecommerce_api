...
import org.example.apispring.model.*;
import org.example.apispring.repository.*;
...
public class OrderService {
    ...
    public Order save(OrderCreationReq req) {
        ...
        for(OrderItemCreationReq item : req.getItems()) {
            ...
            orderItem.setAddress(req.getAddress());
            ...
            if (!notifiedStoreIds.contains(storeId)) {
                notificationService.notifyStoreOwner(storeId, "Bạn có đơn hàng mới từ " + user.getName() + ". Vui lòng chuân bị đơn hàng");
